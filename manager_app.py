...
def api_node_start(node_ref: str):
    try:
        ...
        result = start_managed_node(node_id)
        ok = result.get("status") == "ok"
        record_log(
            f"BẬT node {node_id} -> {'OK' if ok else 'THẤT BẠI'} ({result.get('message', result.get('mode', ''))})",
            "ok" if ok else "err",
        )
...
def api_all_data():
    ...
    # Nếu key chỉ còn ở replica (primary đang down), tra hash ring qua MỘT NODE CÒN SỐNG
    # để hiển thị đúng "node gốc" thay vì dấu hỏi.
    if online_node_ids:
        try:
            route_proxy = connect_node(online_node_ids[0], timeout=max(RPC_TIMEOUT, 2.0))
            for key, entry in all_entries.items():
                if entry.get("primary_node"):
                    continue
                try:
                    route = json.loads(route_proxy.get_routing_info(str(key)))
                    inferred_primary = (route.get("primary") or {}).get("id")
                    if inferred_primary:
                        entry["primary_node"] = inferred_primary
                        entry["primary_inferred"] = True
                except Exception:
                    continue
        except Exception:
            pass
