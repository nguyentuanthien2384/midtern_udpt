...
/* ─── Fetch có timeout (chống treo nút "Đang tải...") ── */
// Nếu manager chậm/treo, request tự hủy sau `ms` ms thay vì quay mãi.
...
async function apiGet(path, ms) {
  const res = await fetchT(API + path, {}, ms || 4000);
  return res.json();
}
...
async function loadClusterStatus(silent) {
  ...
  try {
    var res = await fetchT(API + "/api/cluster/status", {}, 3500);
      ...
      .map(function (n, i) {
        ...
        if (isOnline) {
          ...
          actionsHtml =
            ...
            '<button class="btn btn-sm btn-restore-node" data-node-ref="' +
            escAttr(nodeRef) +
            '" title="Khởi động sạch và lấy lại dữ liệu hiện hành từ cụm" ' +
            'onclick="freshStartNode(this.dataset.nodeRef, this)"><i class="fas fa-rotate-left"></i> Khôi phục</button>';
        } else {
          actionsHtml =
...
async function syncNode(nodeRef, btn) {
  ...
  try {
    var data = await apiPost(
      "/api/sync/" + encodeURIComponent(nodeRef),
      {},
      8000,
    );
    if (data.status !== "ok")
...
async function startNode(nodeRef, btn) {
  ...
  try {
    var data = await apiPost(
      "/api/node/" + encodeURIComponent(nodeRef) + "/start",
      {},
      8000,
    );
    if (data.status !== "ok")
...
async function freshStartNode(nodeRef, btn) {
  if (
    !confirm(
      'Khôi phục node "' +
        nodeRef +
        '"?\n\nNode sẽ khởi động SẠCH: bỏ dữ liệu cũ trên đĩa và lấy lại dữ ' +
        'liệu HIỆN HÀNH từ cụm. Khác với "Bật node" (tiếp tục với dữ liệu cũ).',
    )
  )
    return;
  var origHtml = "";
  ...
  try {
    ...
    if (data.status !== "ok")
      throw new Error(data.message || "Khôi phục node thất bại");
    showToast(
      "Khôi phục " + nodeRef + " thành công (đã lấy lại dữ liệu từ cụm)!",
    );
    addLog("KHÔI PHỤC (sạch) " + nodeRef + " -> OK", "ok");
    setTimeout(function () {
      loadClusterStatus(true);
    }, 150);
    setTimeout(loadRingInfo, 300);
  } catch (e) {
    showToast("Lỗi khôi phục node: " + e, "error");
    addLog("KHÔI PHỤC " + nodeRef + " -> FAIL: " + e, "err");
  } finally {
    if (btn) {
      btn.disabled = false;
      btn.innerHTML = origHtml;
...
async function stopNode(nodeRef, btn) {
  ...
  try {
    var data = await apiPost(
      "/api/node/" + encodeURIComponent(nodeRef) + "/stop",
      {},
      8000,
    );
    if (data.status !== "ok")
...
async function loadAllData() {
  ...
  try {
    var res = await fetchT(API + "/api/all-data", {}, 4000);
    var data = await res.json();
    if (!data.length) {
      tbody.innerHTML =
