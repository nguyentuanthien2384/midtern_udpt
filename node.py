...
class KeyValueNode:
    ...
    def get_routing_bulk(self, keys_json: Any) -> str:
        """Tra primary cho NHIỀU key trong một lời gọi (giảm round-trip cho UI)."""
        try:
            keys = json.loads(keys_json) if isinstance(keys_json, str) else list(keys_json)
        except Exception:
            keys = []
        out = {}
        for key in keys:
            try:
                out[str(key)] = self._get_primary_node(str(key)).id
            except Exception:
                out[str(key)] = None
    ...
    def _detect_failures_loop(self) -> None:
        while True:
            ...
            # Bảo trì replica định kỳ (mỗi vòng ~2s), tự hội tụ:
            #  - Có node chết  -> BÙ bản thiếu để mỗi key đủ replication_factor bản sống.
            #  - Cụm khỏe mạnh -> DỌN bản thừa (bản bù tạm) về đúng số bản.
            threading.Thread(target=self._maintain_replication, daemon=True).start()
            time.sleep(2)

    def _maintain_replication(self) -> None:
        """Điều phối bảo trì replica: bù khi thiếu, dọn khi thừa."""
        with self.lock:
            has_dead = any(s == "DEAD" for s in self.node_status.values())
        if has_dead:
            self._replenish_replicas()
        else:
            self._reconcile_strays()

    def _replenish_replicas(self) -> None:
        """BÙ bản sao thiếu khi cụm đang có node chết.

        Mỗi key node này đang giữ phải đủ `replication_factor` bản trên các NODE
        CÒN SỐNG. Nếu một chủ-sống nào đó chưa có bản, node này đẩy thêm sang.
        Chạy định kỳ nên không phụ thuộc việc bắt đúng khoảnh khắc node chết
        (vá trường hợp 'lỡ nhịp' khiến key kẹt ở 1 bản).
        """
        ...
        try:
            ...
            for key, value in snapshot.items():
                ...
                if self.node_id not in live_ids:
                    continue  # node sống khác chịu trách nhiệm key này
                for o in live:
            ...
            if pushed:
                self._persist()
                print(f"[{self.node_id}] BÙ {pushed} bản sao thiếu (có node chết)")
        finally:
